...
from CarlaEnv.utils import HParamCallback, TensorboardCallback, write_json, VideoRecorder

from CarlaEnv.config import CONFIG
try:
...
class CarlaRouteEnv(gym.Env):
    ...
    def __init__(self, host="127.0.0.1", port=2000,
                 viewer_res=(500, 350), obs_res=(160, 80),
                 reward_fn=None,
        ...
        try:
            ...
            self.dashcam_seg = Camera(self.world, out_width, out_height,
                                      ...
                                      on_recv_image=lambda e: self._set_observation_seg_image(e),
    ...
    def reset(self, is_training=False):
        ...
        self.observation_seg = self.observation_buffer_seg = None  # Last received observation
    ...
    def render(self, mode="human"):
        ...
        pos_observation_seg = (self.display.get_size()[0] - obs_w - 10, 10 + 2 * obs_h)
        self.display.blit(pygame.surfarray.make_surface(self.observation.swapaxes(0, 1)), pos_observation)
        self.display.blit(pygame.surfarray.make_surface(self.observation_seg.swapaxes(0, 1)), pos_observation_seg)

        pos_vae_decoded = (self.display.get_size()[0] - obs_w - 10, 10 + obs_h)
        ...
        text_surface = self.font.render("RGB Camera", True, (255, 255, 255))
        self.display.blit(text_surface, (self.display.get_size()[0] - obs_w - 8, 12))

        text_surface = self.font.render("VAE", True, (255, 255, 255))
        self.display.blit(text_surface, (self.display.get_size()[0] - obs_w - 8, 12 + obs_h))

        text_surface = self.font.render("GT", True, (255, 255, 255))
        self.display.blit(text_surface, (self.display.get_size()[0] - obs_w - 8, 12 + 2 * obs_h))
    ...
    def step(self, action):
        ...
        self.observation_seg = self._get_observation_seg()
        if self.activate_spectator:
    ...
    def _get_observation_seg(self):
        while self.observation_seg_buffer is None:
            pass
        obs = self.observation_seg_buffer.copy()
        self.observation_seg_buffer = None
        return obs
    ...
    def _set_observation_seg_image(self, image):
        self.observation_seg_buffer = image
...
vae = load_vae(f'/home/albertomate/Documentos/carla/PythonAPI/my-carla/vae/log_dir/{CONFIG["vae_model"]}', LSIZE)
...
video_path = "/home/albertomate/Documentos/carla/PythonAPI/my-carla/linkedin.avi"
record_video = True
if record_video:
